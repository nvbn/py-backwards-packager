...
def _create_empty_packages(setup_kwargs, tmp_dir):
    for package in setup_kwargs['packages']:
        path = os.path.join(tmp_dir, package.replace('.', '/'))
        try:
            os.makedirs(path)
        ...
        with open(os.path.join(path, '__init__.py'), 'w') as f:
            f.write('# Autogenerated for keeping package structure')
...
def _compiled(setup_kwargs, targets):
    with TemporaryDirectory(dir='.') as tmp_dir:
        _create_empty_packages(setup_kwargs, tmp_dir)
